...
const prossessRegionData = (provinceName , cityName , countryName) => {
  ...
  if(data && provinceName){
    province = _.head(_.filter(data , (item) => {return item.Name === provinceName && item.LevelType === 1 }));
    if(province && cityName){
      if(cityName === '省直') {
        city = { ID : -10 }
      } else {
        city = _.head(_.filter(data  , (item) => {return item.Name === cityName && item.LevelType === 2  }));
      }
      if(city && countryName){
        if(countryName === '市直') {
          country = { ID : -100 }
        } else {
          country = _.head(_.filter(data , (item) => {return item.Name === countryName && item.LevelType === 3  }));
        }
